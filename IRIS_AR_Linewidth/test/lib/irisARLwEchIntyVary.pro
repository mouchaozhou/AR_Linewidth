pro irisARLwEchIntyVary, nStCaseNum, nEdCaseNum, sEchWthSavPartDirName, sEchFitTauSavDir, $
  ...
  for i = nStCaseNum, nEdCaseNum do begin
      ...
      aEchInty = dblarr(nNCase) -2D      ; The total intensity of each AR
      ...
      for j = aEvtNums[0], aEvtNums[1] do begin
          ...
          aEchInty[j - aEvtNums[0]] = nEchInty             
      endfor

      aValidIndex  = where((aEchInty ge uImgPars.aYThresh[0]) and (aEchInty le uImgPars.aYThresh[1]))

      if (aValidIndex[0] ne -1) then begin
          aMu = aMu[aValidIndex]
          aEchInty = aEchInty[aValidIndex]
          ...
          ; Normalise the data
          aEchInty /= mean(aEchInty[aMuRgId])
          
          ; #test
          ;  aEchInty /= min(aEchInty)
          ; #endtest
          
          ; Save the original data
          mo_push, aMus, aMu
          mo_push, aEchIntys, aEchInty 
      endif 
  endfor
  ...
  plot, aMus, aEchIntys, /xstyle, /ystyle, psym=nPsym, xrange=aDfXrange, xtitle='!7l!X', ytitle='I(!7l!X)/I(1)', $
