pro irisARLwAvgIntyVarChk, nStCaseNum, nEdCaseNum, sAvgWthSavPartDirName, sAvgFitTauSavDir, $
                           sAvgIntyVarChkImgDir, sLineID, uImgPars
  ...
  for i = nStCaseNum, nEdCaseNum do begin
      ...
      if (aValidIndex[0] ne -1) then begin
          ...
          plot, aMus, aAvgIntys, /xstyle, /ystyle, psym=nPsym, xrange=aDfXrange, xtitle='!7l!X', ytitle='I(!7l!X)/I(1)', $
            yrange=[uImgPars.aYrange[0]*0.95, uImgPars.aYrange[1]*1.05], position=aPosition, title=num2str(i), $
          ...
          oplot, aMu, aAvgInty, color=mo_loadColour('Yellow'), psym=nPsym        
          ...
          write_png, sAvgIntyVarChkImgDir + string(i) + '.png', tvrd(/true)
      endif 
  endfor
  ...
  ; Make movies
  mo_makeGifMovie, sAvgIntyVarChkImgDir + '*.png', sAvgIntyVarChkImgDir + 'intensity_variation_check.gif'
